 ...
 * Author      : Dustin MIner
 * Description : A class that converts temperatures
 */
...
// CLASS FUNCTION DEFINITIONS GO HERE
 /*
  ...
  {
      stringstream sstemp;
      sstemp.setf(std::ios::fixed|std::ios::showpoint);
      sstemp.precision(2);
      switch (toupper(unit))
      {
          case 'F' :
          
          sstemp << GetTempAsFahrenheit() << " Fahrenheit";
          
          break;
          
          case 'C' :
          
          sstemp << GetTempAsCelsius() << " Celsius";
          
          break;
          
          case 'K' :
          
          sstemp << GetTempAsKelvin() << " Kelvin";
